-- route_update : échec explicite si aucune ligne n’est mise à jour (id invalide, inactive, etc.).
-- Évite un « ok: true » alors que rien n’a changé en base.
...
COMMENT ON FUNCTION public.route_update(text, text, text, text, jsonb, smallint) IS
  'Met à jour une route active : sorties custom ou falaises / brehec / boucle. JWT admin requis. Échoue si 0 ligne mise à jour.';
