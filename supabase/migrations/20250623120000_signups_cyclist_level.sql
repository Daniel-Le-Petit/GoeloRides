-- Niveau cycliste par inscription + affichage dans signup_list_all_names.
-- À appliquer après 20250621130000_signup_waitlist_route_visibility.sql (remplace à nouveau signup_register / signup_list_all_names).

ALTER TABLE public.signups
  ADD COLUMN IF NOT EXISTS cyclist_level text;

COMMENT ON COLUMN public.signups.cyclist_level IS
  'Niveau déclaré à l''inscription (ex. debutant, intermediaire, confirme) — affiché avec le pseudo.';
...
BEGIN
  ...
  INSERT INTO public.signups (route_id, pseudo, email, waitlist, cyclist_level)
  VALUES (v_rid, v_pseudo, v_email, v_waitlist, v_cl);
...
REVOKE ALL ON FUNCTION public.signup_register(text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.signup_register(text, text, text, text) TO anon, authenticated, service_role;
