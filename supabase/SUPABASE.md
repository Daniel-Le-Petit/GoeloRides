...
**Déploiement :** envoie aussi **`parcours.js`**, **`sortie.js`**, **`sorties.js`**, **`goelo-auth.js`**, **`goelo-home-perimeter-map.js`** et **`parcours.css`** avec `index.html`, `sorties.html`, `sortie.html` et `groupes.html` (même dossier à la racine du site).
