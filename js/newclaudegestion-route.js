/**
 * GoëloRides — js/gestion-sorties.js
 * Gestion des sorties : création (?mode=create) et édition (?mode=edit&id=xxx)
 *
 * Bugs corrigés :
 *  - SyntaxError "params already declared" → params déclaré UNE seule fois dans DOMContentLoaded
 *  - "No API key found" → Supabase initialisé APRÈS chargement UMD, jamais avant
 *  - 406 Not Acceptable → header Accept ajouté + .single() correctement chaîné
 *  - getSb is not defined → singleton défini en haut de fichier, avant tout usage
 *  - race condition UMD → await Promise sur script.onload avant createClient
 *  - duplication mode/routeId → tout sur window.*, une seule assignation
 *  - publishSortie → "if (error)" → corrigé en "if (result.error)"
 *  - loadGpx → "document.getElementById('px-status')" → corrigé en 'gpx-status'
 */

console.log('🔥 Claude -gestion-sorties.js LOADED — ' + new Date().toISOString());
console.log("JS START", {
  url: window.GOELO_SUPABASE_URL,
  key: window.GOELO_SUPABASE_ANON_KEY
});
console.log("INIT SCRIPT", new Date().toISOString());
window.__DEBUG_GS__ = true;

/* ═══════════════════════════════════════════════════════════════
   ÉTAT GLOBAL (une seule déclaration de chaque variable)
   ═══════════════════════════════════════════════════════════════ */
window.mode    = 'create'; // 'create' | 'edit'
window.routeId = null;     // id Supabase de la route en cours d'édition

window.currentEmbeddedPoints = []; // points GPX [{lat, lng, ele}]
window.currentCoverImage     = ''; // dataURL image flyer

/* ═══════════════════════════════════════════════════════════════
   SUPABASE — singleton robuste
   Aucune requête ne peut être faite avant que _sb soit initialisé.
   ═══════════════════════════════════════════════════════════════ */
...
async function getSb() {
  ...
  if (window.__GOELO_SB__) {
    return window.__GOELO_SB__;
  }

  console.log("getSb - SUPABASE URL =", window.GOELO_SUPABASE_URL);
  console.log("getSb - SUPABASE KEY =", window.GOELO_SUPABASE_ANON_KEY);

  const url = window.GOELO_SUPABASE_URL  || '';
  const key = window.GOELO_SUPABASE_ANON_KEY || '';

  if (!url || !key) {
    throw new Error('Supabase config manquante (URL ou ANON_KEY non définis)');
  }

  // Charger le SDK UMD si pas encore disponible
  if (!window.supabase || typeof window.supabase.createClient !== 'function') {
   ...
   if (existing) {
     ...
     // Le script existe mais n'a peut-être pas fini de charger
   ...
   s.onload  = () => {
     console.log('✅ Supabase UMD chargé');
     resolve();
   };
  ...
  const sb = window.__GOELO_SB__;
  console.log("DEBUG sb =", sb);
  console.log("DEBUG typeof sb =", typeof sb);
  console.log("DEBUG keys =", Object.keys(sb || {}));
  console.log("DEBUG from =", sb?.from);
...
/* ═══════════════════════════════════════════════════════════════
   BUILD PAYLOAD
   Construit l'objet pour route_create / route_update
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   SAVE DRAFT — create ou update selon window.mode
   ═══════════════════════════════════════════════════════════════ */
async function saveDraft() {
  ...
  try {
    ...
    if (window.mode === 'edit' && window.routeId) {
      ...
      // Passer en mode edit pour les sauvegardes suivantes
      if (result?.data) {
...
/* ═══════════════════════════════════════════════════════════════
   PUBLISH — create ou update selon window.mode
   ═══════════════════════════════════════════════════════════════ */
async function publishSortie() {
  ...
  try {
    ...
    // BUG CORRIGÉ : était "if (error)" → doit être "if (result.error)"
    if (result?.error) throw result.error;
...
function duplicateSortie() {
  ...
  // Repasser en create pour générer un nouvel id
  window.mode    = 'create';
  window.routeId = null;
...
/* ═══════════════════════════════════════════════════════════════
   LOAD ROUTE (mode edit)
   Fetch depuis Supabase + injection dans le formulaire
   ═══════════════════════════════════════════════════════════════ */
let __loadRouteRunning = false;

async function loadRoute(id) {
  if (__loadRouteRunning) {
    console.warn("loadRoute bloqué (déjà en cours)");
    return;
  }
  ...
  try {
    ...
    const { data, error } = await sb
      .from('routes')
      .select('id, track_name, group_label, pace_label, sort_order, route_kind, front_config')
      .eq('id', id)
      .maybeSingle();

    console.log('📥 loadRoute data:', data);
    console.log('📥 loadRoute error:', error);

    if (error) throw error;
    if (!data) {
      showToast('Sortie introuvable (id: ' + id + ')', 'error');
      return;
    }
...
function populateForm(route) {
  ...
  if (hRdvEl) hRdvEl.value = fc.meetTime || fc.rideTime || '08:30';
  ...
  if (capEl) capEl.value = fc.captain || '';
  ...
  if (Array.isArray(fc.embeddedPoints) && fc.embeddedPoints.length > 0) {
    ...
    // Profil altimétrique si altitudes disponibles
    const hasEle = coords.some(c => c.ele !== 0);
    if (hasEle) drawElevation(coords);
  }

  // Image flyer si enregistrée
...
/* ═══════════════════════════════════════════════════════════════
   GPX IMPORT
   BUG CORRIGÉ : 'px-status' → 'gpx-status'
   ═══════════════════════════════════════════════════════════════ */
let gpxMap = null;
...
function parseGpx(xml, filename) {
  ...
  // Stocker pour Supabase (format [[lat,lng,ele]])
...
function onFlyerBgSelected(input) {
  ...
  reader.onload = function (e) {
    ...
    img.onload = function () {
      _flyerBgImage           = img;
      window.currentCoverImage = e.target.result; // stocker pour Supabase
      const nameEl = document.getElementById('flyer-bg-name');
...
function generateFlyer() {
  ...
  // Logo
  ctx.textAlign = 'left';
  ctx.fillStyle = '#C8F135'; ctx.font = '700 40px Arial';
  ctx.fillText('≋', 48, 88);
