...
(function () {
  ...
  function rowHtml(s) {
    ...
    var dateBlock = d
      ? '<span class="so-date__wd">' + WEEKDAY_SHORT[d.getDay()] + "</span>" +
        '<span class="so-date__num">' + d.getDate() + "</span>" +
        '<span class="so-date__mo">' + MONTH_SHORT[d.getMonth() + 1] + "</span>"
      : '<span class="so-date__wd">—</span><span class="so-date__num">?</span><span class="so-date__mo"></span>';
    ...
    var time = d
      ? String(d.getHours()) + "h" + String(d.getMinutes()).padStart(2, "0")
      : "";
    ...
    var manageBtns = state.isTeamRider
      ...
      : '<button type="button" class="so-act is-locked" data-lock="manage" aria-disabled="true">' +
        '<span aria-hidden="true">🔒</span> Modifier</button>' +
        '<button type="button" class="so-act is-locked" data-lock="manage" aria-disabled="true">' +
        '<span aria-hidden="true">🔒</span> Annuler</button>';

    var joinBtn = state.isTeamRider
      ? '<a class="so-act" href="' + escapeAttr(detailHref) + '">Rejoindre</a>'
      : '<button type="button" class="so-act is-locked" data-lock="join" aria-disabled="true">' +
        '<span aria-hidden="true">🔒</span> Rejoindre</button>';

    return (
      '<li><article class="so-row' + (cancelled ? " is-cancelled" : "") + '">' +
      '<div class="so-date so-date--' + tone + '">' + dateBlock + "</div>" +
      '<div class="so-body">' +
      '<h2 class="so-title">' +
      escapeHtml(s.title) +
      ...
      '<p class="so-meta">' +
      '<span><span aria-hidden="true">📍</span> ' + escapeHtml(s.place) + "</span>" +
      (time ? '<span><span aria-hidden="true">🕒</span> ' + time + "</span>" : "") +
      "</p>" +
      "</div>" +
      '<div class="so-people">' + avatarsHtml(s.participants) + "</div>" +
